...
交接物回到场景稿本身（逐字台词、动作行、头部注明锁定句与来源）；生产版设计表由"对白场进入生产默认附"改为**按需**（逐句诊断或制作方明确要求时从场景稿逐字抽出）。理由：同日实践证明先填表会让句与句不推，而场景稿已承担台词锁、来源与表演信息，同项目金样（EP01）也没有表；2.1.0 把表设为默认是过度设计，本版收回。抽取顺序与生动自查行保留在模板里供按需使用。SKILL 与交接契约同步。
