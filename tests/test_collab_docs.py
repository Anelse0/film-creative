...
class CollabDocsTests(unittest.TestCase):
    ...
    def test_handoff_contract_is_identical_to_film_director_copy_when_present(self):
        sibling = ROOT.parent / 'film-director' / 'references' / 'handoff-contract.md'
        if sibling.exists():
            self.assertEqual((ROOT / 'references/handoff-contract.md').read_bytes(), sibling.read_bytes())
