...
- **台词经济**：接住上一句只是必要条件，每句还要带来一样（信息 / 要或拒 / 关系 / 笑点或性格）；观众已知的不再说，简单的事一两句说完。写完连续片段做删除测试（逐句）与压缩测试（逐段，先写"这段要改变的一件事"），对照前后场，记在剧本页 `### 删除测试`（`references/stage-3c-script.md` §3c.5）。
- `scripts/review_script.py` 总判断三项：对白连通 / 台词经济 / 事件轨。台词经济只把"缺记录、记录与正文不符"判为问题；接话 / 重复 / 递话问句 / 截断候选与低信息段附"前文哪里说过、后文哪里回扣"（新参数 `--later`）交模型复核——10 场盲评标注上表面规则精确率 0.32，判不了废话。去掉三处推着补接话的提示："有人接的台词占比"、"全场没有一个问句"、说出口行的"（没人接）"。来源：Scriptnotes 609 后半（S5）。
...
- 场面轨交给 film-director 改为**告知，不锁定**：分镜可以改分段、子空间、活动乃至地点，在方案里写一行与场面轨的差异；台词仍锁定。分镜时长超出场面轨总估时 20% 以上只提醒，不退回，回不回剧本层由用户定。
...
- `scripts/review_script.py` 新增"潜台词无支点"检查：省略句里当作已知的设定（你的 X / 那个 X / 你有 X / 还 X）前文没建立、接下来也没人问 → 报问题并附三种改法（直说来历 / 用当场动作引出 / 让第三人替观众问），不改稿；`--context` 传前几场建立"前文"。来源：Clark & Brennan 1991 共同基础、SSJ 1974 收件人设计、Scriptnotes 693。用 EP02 场 3 钥匙段（修复前 / 修复后）校准。
