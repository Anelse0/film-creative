...
需要分析表时可裁剪 `templates/dialogue-design-sheet.md`，不默认逐句填写。对白场进入生产时改用同一模板的生产版（每句一行、逐字、来源标注）作为交接物，见 `references/handoff-contract.md`。生产版从已写活的连续交流中逐字抽出，不先填表再配句。已确认偏好见 `references/preference-ledger.md`；没有项目证据的旧偏好不能当新项目要求。
